package jp.ac.kyushu.ait.posl.modules.refactoring.trace;
...
public class RenameSpecifier {
    ...
    public static void linkMethods(Structure analyzerX_1, Structure analyzerX, List<Refactoring> refactoringInstances) {
     ...
     * @param right
     */
    private static void renameClass(Structure analyzerX, Structure analyzerX_1, List<CodeRange> left, List<CodeRange> right) {
        assert (left.size() == 1);
        assert (right.size() == 1);
        String leftPathName = left.get(0).getFilePath();
        String rightPathName = right.get(0).getFilePath();
     ...
     * @param right
     */
    private static void signatureChange(Structure analyzerX, Structure analyzerX_1, List<CodeRange> left, List<CodeRange> right) {
        assert (left.size() == 1);
        assert (right.size() == 1);
        String leftPathName = left.get(0).getFilePath();
        int leftNo = left.get(0).getStartLine();
        ...
        rightMethod.methodInfo.anotherSignature = rightMethod.methodInfo.anotherSignature.replace(rightMethod.methodInfo.signature, leftMethod.methodInfo.signature);
        leftMethod.methodInfo.anotherSignature = leftMethod.methodInfo.anotherSignature.replace(leftMethod.methodInfo.signature, rightMethod.methodInfo.signature);
     ...
     * @param to
     */
    private static void renameAllSignature(Structure analyzer, String from, String to) {
        List<String> signatures = analyzer.getSignatures(from);
        for (String s : signatures) {
            MethodDefinition md = analyzer.getMethod(s);
            md.methodInfo.anotherSignature = md.methodInfo.anotherSignature.replace(from, to);
